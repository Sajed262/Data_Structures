...
private:
    ...
    DynamicArray<List<int>> edges;     // if the graph is direct each cell in the array will be vector that contains the IN edges
    DynamicArray<List<int>> out_edges; // will be used only if the graph is directed.
    // e.g., if we have an edge 1->2, then in the out_edges at index 1 we will add 2,
    // and in the edges at index 2 we will add 1.
...
public:
    ...
    explicit Graph(bool directed = true) : is_directed(directed), num_of_vertices(0), edges(), out_edges(), weighted_edges() {}
    ...
    Graph(int vertices, bool is_directed, vector<Edge> ed) : num_of_vertices(vertices), is_directed(is_directed),
                                                             edges(num_of_vertices), out_edges(num_of_vertices), weighted_edges(ed)
    {
    ...
    // we dont add the same edge more than once. lets we have 1->2 and 2->1 in the directed graph and we want the undirected graph of it,
    ...
    void addEdge(int from, int to, int wei = 1)
    {
        if (from < 0 || to < 0 || from >= num_of_vertices || to >= num_of_vertices)
            return;
        if (edges[to].getElement(from)) // the case of parallel edges.
            return;

        edges[to].addElement(from, wei);
        if (is_directed)
            out_edges[from].addElement(to, wei);
    ...
    void removeEdge(int from, int to)
    {
        ...
        if (is_directed)
            out_edges[from].removeElement(to);
        else // the graph is undirected
